...
Item {
        ...
        Item {
            ChartView {
                ...
                Component.onCompleted: {
                    var xlist = xAxisTime()
                    var series = chartView2.createSeries(ChartView.SeriesTypeLine, "Import Money", xAxis, yAxis);
                    for(var i = 0; i < xlist.timeList.length; i++){
                        series.append(xlist.timeList[i], xlist.valueList[i])
                    }
                    var list = dboperator.searchExport_Price_Time()
                    var series2 = chartView2.createSeries(ChartView.SeriesTypeLine, "Export Money", xAxis, yAxis);
                    for(var j = 0; j < list.length; j++){
                        series2.append(list[j].getTime, list[j].getTotalprice)
                    }
                }
    ...
    function totalPrice(){
        var totalPriceList = []
        var list = dboperator.sortExport("eserialID")
        for(var i = 0; i < list.length; i++){
            totalPriceList[i] = list[i].getTotalprice.toString()
        }
    ...
    function xAxisList(){
        ...
        var list = dboperator.sortExport("eserialID")
        for(var i = 0; i < list.length; i++){
            xAxisList[i]=list[i].getEserialID
        }
        return xAxisList
    }
    function xAxisTime(){
